...
public class PostController {
    ...
    public ResponseEntity<List<JsonNode>> posts() {
        List<JsonNode> sortedPosts = postRepository.findAll().stream()
                .map(Post::asJson)// sort by id descending (or createdAt descending if you have a timestamp)
                .sorted((a, b) -> {
                    long idA = a.get("id").asLong();
                    long idB = b.get("id").asLong();
                    return Long.compare(idB, idA); // reverse order
                })
                .toList();
